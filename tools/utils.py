...
def build_optimizer_schedular(cfg, param_groups, train_loader, train_set):
    
    if cfg.TRAIN.OPTIMIZER.TYPE.lower() == 'sgd':
        optimizer = torch.optim.SGD(param_groups, momentum=cfg.TRAIN.OPTIMIZER.MOMENTUM, nesterov=cfg.TRAIN.OPTIMIZER.NESTEROV)
    ...
    if cfg.TRAIN.LR_SCHEDULER.TYPE == 'plateau':
        ...
        lr_scheduler = MultiStepLR(optimizer, milestones=cfg.TRAIN.LR_SCHEDULER.LR_STEP, gamma=cfg.TRAIN.LR_SCHEDULER.FACTOR)
    ...
    elif cfg.TRAIN.LR_SCHEDULER.TYPE == 'cosine_annealing': 
        lr_scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(optimizer, len(train_set)*cfg.TRAIN.MAX_EPOCH)
    else:
